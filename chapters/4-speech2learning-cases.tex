...
\begin{figure}[htb]
\centering
\caption{1ª Instância da \textit{Speech2Learning}: API REST para Legendar Videoaulas}
...
1 & pt-BR & BRA & M & Apps Android & 0:17 \\
2 & pt-BR & BRA & M & SCRUM & 0:26 \\
3 & pt-BR & BRA & F & Selenium WebDriver & 0:23 \\
4 & pt-BR & BRA & F & Blockchain & 0:20 \\
5 & pt-BR & BRA & M & Kernel Híbrido & 0:24 \\
6 & en-US & BRA & F & Visto de Trânsito & 0:29 \\
7 & en-US & USA & Ambos & Entrevista de Emprego & 0:16 \\ 
8 & en-US & BRA & F & Oportunidades de Emprego & 0:20 \\ 
9 & en-US & BRA & F & Liderança Servidora & 0:15 \\ 
10 & en-US & BRA & M & Goroutines & 0:15 \\ 
11 & es-AR & ARG & F & Lógica de Programação & 0:12 \\ 
12 & es-AR & ARG & F & Linguagens de programação & 0:21 \\ 
13 & es-AR & ARG & F & Tipos de Dados em Python & 0:14 \\ 
14 & es-AR & ARG & F & Hello World com Python & 0:17 \\ 
...
Nas subseções seguintes, delineamos cada fonte de evidência e suas técnicas de coleta, esclarecendo todo o processo de triangulação e sua contribuição para uma compreensão robusta do impacto das tecnologias ASR, como o STT, na acessibilidade educacional.
...
Medir a semelhança entre textos é uma prática comum em pesquisas acadêmicas, geralmente apoiada por métodos de análise léxica \cite{Majumdar2022}. Para comparar nossas transcrições automáticas e de referência, precisamos definir como extrairemos nossos dados quantitativos utilizando técnicas de similaridade léxica. No nosso contexto, algumas das alternativas mais interessantes são:
...
Uma revisão de literatura complementa nossos achados empíricos ao identificar estudos relevantes sobre tecnologias ASR e STT. Esta revisão apoia a contextualização e o enriquecimento dos dados quantitativos, proporcionando uma compreensão mais profunda do potencial e das limitações das tecnologias atuais baseadas em ASR.

Empregamos a Teoria Fundamentada \cite{Charmaz2009} como um \textit{framework} orientador para nossa análise documental. A Teoria Fundamentada é uma metodologia sistemática para a análise de dados qualitativos com o objetivo de desenvolver teorias, permitindo o surgimento de temas e padrões a partir dos próprios dados, em vez de impor noções preconcebidas.

Nossa Pesquisa Documental envolveu uma revisão abrangente da literatura relevante, incluindo artigos acadêmicos, livros e relatórios, focando nas tecnologias ASR e STT no contexto da acessibilidade educacional. Através desta análise, buscamos identificar temas recorrentes, guias teóricos e lacunas nas pesquisas existentes, contribuindo para uma compreensão mais detalhada do assunto.
...
\begin{itemize}
\item \textbf{Questão de Pesquisa}: Como as tecnologias baseadas em ASR e STT estão influenciando a acessibilidade na área da educação?
\end{itemize}

Essa combinação de hipóteses quantitativas e uma questão de pesquisa qualitativa permite uma triangulação robusta de dados, fornecendo uma visão abrangente e multidimensional do impacto das tecnologias de ASR/STT na acessibilidade educacional. A análise das respostas do \textit{Survey}, juntamente com os métodos de similaridade léxica e a pesquisa documental, nos permitirá corroborar os achados, identificar as convergências entre nossas fontes de evidências e enriquecer as discussões, fortalecendo a validade do estudo.

\subsection{Resultados e Discussões}

\subsubsection{Métodos de Similaridade Léxica}

Sabemos que o \textit{dataset} quantitativo deste Estudo de Caso foi construído para avaliar a qualidade das transcrições automáticas dos serviços de ASR/STT oferecidos por provedores líderes como Amazon, Google, IBM, Microsoft e OpenAI. Esta avaliação utilizou três métodos de similaridade léxica: CS, JI e LD. Nesse sentido, considerando as características dos dados coletados, focamos no JI devido ao seu desempenho estatístico mais robusto, sendo nossa única amostra com distribuição normal.

Apenas relembrando, o JI mede a similaridade entre dois conjuntos de dados calculando o tamanho da interseção dividido pelo tamanho da união dos conjuntos de amostras. No nosso contexto, ele compara as palavras da transcrição automática com as da transcrição de referência (revisada por especialistas), proporcionando uma medida quantitativa de precisão.
...
\includegraphics[width=0.95\textwidth]{images/chapter4-cs1-lexical-by-lang-barplot.png}
\fadaptada{FalvoJr2023_HICSS}
\end{figure}
...
Esses resultados destacam diferenças significativas na qualidade das transcrições automáticas dos provedores avaliados em relação à similaridade léxica. Além disso, abrem caminho para uma exploração mais aprofundada de como essas características podem impactar a experiência do usuário em várias aplicações dessas transcrições. Isso inclui a satisfação do aluno e a percepção sobre a precisão da transcrição em diferentes contextos linguísticos. Esses pontos são discutidos na subseção seguinte com base nos resultados do \textit{Survey}.

\subsubsection{Respostas do \textit{Survey}}

Com base nos \textit{insights} do estudo de similaridade léxica, esta seção apresenta os resultados de uma pesquisa anônima baseada nas percepções dos participantes sobre a qualidade das transcrições automáticas, explorando o mesmo \textit{dataset} definido para as fontes de evidência quantitativas. Essa pesquisa teve como objetivo correlacionar os dados de similaridade léxica com as avaliações subjetivas de pessoas reais sobre a qualidade das transcrições dos diferentes provedores de serviços de ASR/STT.
...
Dada a distribuição não normal dos dados, o teste de \textit{Dunn} \cite{Dunn1964} foi escolhido em detrimento do teste de \textit{Conover} \cite{Conover1999} para a análise \textit{post-hoc} devido às suas suposições menos rigorosas sobre a distribuição dos dados e sua capacidade de lidar efetivamente com dados que possuem \textit{outliers} (\autoref{tab:chapter4-results-survey-tests}), embora ambos tenham produzido resultados análogos em testes informais. 

O teste de \textit{Dunn} comparou pares de provedores, indicando diferenças significativas entre quase todos os pares. Notavelmente, todas as comparações envolvendo OpenAI e outros provedores demonstraram diferenças significativas, sugerindo uma virtual superioridade desse provedor.
...
Esses \textit{insights} são cruciais para provedores que buscam otimizar seus serviços de transcrição para conteúdos educacionais, pois ilustram a importância tanto da precisão linguística quanto da percepção do usuário na avaliação da qualidade das transcrições. As descobertas sugerem um roteiro para futuras melhorias e a potencial customização de serviços para atender de forma mais eficaz às diversas necessidades linguísticas.
...
O estudo de \citeonline{Ferraro2023} apresenta uma investigação extensa sobre a transcrição de linguagem falada usando modelos de ML. Sua análise compara serviços de STT de código aberto e pagos, com foco na qualidade e variedade de entrada. A pesquisa utiliza diversos conjuntos de dados de entrevistas, palestras e discursos, empregando métricas como a Taxa de Erro de Palavras (WER) para avaliação. O trabalho de \citeonline{Ferraro2023} fornece um ponto de referência, alternativo aos métodos de similaridade léxica, para avaliar a precisão da transcrição, estabelecendo uma base sólida para trabalhos futuros.
...
\citeonline{Homburg2019} investigam o uso de robôs humanoides como avatares para tradução de linguagem de sinais, visando melhorar a inclusão para a comunidade surda. Diferente de pesquisas anteriores focadas apenas no reconhecimento de linguagem de sinais, seu estudo explora uma abordagem inovadora ao empregar robôs como intermediários para comunicação em linguagem de sinais. Ao entrevistar 50 participantes surdos, eles avaliam a eficácia percebida dos robôs humanoides na facilitação da tradução de linguagem de sinais, oferecendo \textit{insights} inovadores para soluções de acessibilidade.
...
\begin{itemize}
\item \textbf{Avaliação da Qualidade de Transcrição (AQT)}: Avaliação do desempenho e qualidade das tecnologias baseadas em ASR/STT em tarefas de transcrição. Esta categoria foca na precisão das transcrições, utilizando métricas como a Taxa de Erro de Palavras (WER) e comparações entre diferentes serviços de STT, sejam eles de código aberto ou pagos.
...
\item \textbf{Tecnologias Assistivas (TAs)}: Investigação de abordagens inovadoras, como robôs humanoides e ferramentas de aprendizagem multimodal, para melhorar a acessibilidade para diversos alunos. Esta categoria abrange o uso de tecnologias avançadas para suportar a inclusão de alunos com necessidades especiais, como a tradução/sinalização de línguas de sinais.

\item \textbf{Desafios e Oportunidades (DO)}: Identificação de desafios e oportunidades para melhorar a precisão e eficácia do ASR em contextos educacionais. Este tema envolve a análise das dificuldades encontradas na implementação de tecnologias ASR em ambientes ruidosos e colaborativos, além de identificar possíveis melhorias e inovações para superar essas barreiras.
\end{itemize}
...
\begin{tabular}{c|p{6.8cm}|p{5.7cm}}
...
\item \textbf{Similaridade Léxica vs. Percepções dos Usuários}: Os dados de similaridade léxica revelaram que diferentes provedores de STT têm diferenças estatisticamente significativas na precisão das transcrições, conforme medido pelo JI. Notavelmente, a OpenAI mostrou desempenho superior em vários idiomas comparado a outros provedores importantes como Google e IBM. No entanto, os resultados do \textit{Survey}, que também focaram em avaliações quantitativas, mas usando escala Likert, indicam que a satisfação do usuário é influenciada por mais do que apenas a precisão léxica. Embora a OpenAI também tenha alcançado a maior satisfação entre os participantes, as diferenças estatísticas nas respostas do \textit{Survey} sugerem que as percepções dos usuários são influenciadas por fatores como compreensão do contexto e tolerância a erros, que não são totalmente capturados pelos métodos de similaridade léxica.
